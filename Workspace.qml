// Shared application state and UI composition. Launchers supply windows
// and platform services; providers own note storage and backend behavior.
...
Item {
  ...
  property Item contentHost: root
  ...
  readonly property color sourceInk: sourceBase.a > 0 ? Qt.tint(foreground, Util.alpha(sourceBase, TabColors.inkAlpha())) : foreground
  ...
  readonly property var services: ({
    ...
    "processes": {
      "create": function(owner) {
        return processRunnerComponent.createObject(owner);
      }
    },
    "microsoft": {
      "create": function(owner, scopes, clientId) {
        return root.createMicrosoftAccount(owner, scopes, clientId);
      }
    },
    "requests": {
      "queueFor": function(key, provider) {
        return root.queueFor(key, provider);
      },
      "cancelOwner": function(owner) {
        root.cancelQueuedFor(owner);
      }
    }
  })
  // ── request queues ──────────────────────────────────────────────────
  ...
  property var lastNotes: ({
  })
  // section key -> the note last open there
  ...
  // "No match".
  readonly property bool searchBusy: root.filterText.length >= 2 && root.searchContent && (contentSearchTimer.running || Object.keys(root.searchWaiting).length > 0)
  ...
  function open(payloadJson) {
    ...
    if (!root.accounts.some(function(a) {
      return a.loggingIn;
    }))
      editor.clearNotice();
  ...
  function releaseProviders() {
    // Dispose dependents while the workspace and its request lanes still
    // have live QML contexts. Their read processes stop with their owners.
    var owned = root.providers.slice();
    for (var i = 0; i < owned.length; i++) {
      root.retireProvider(owned[i]);
    }
  }
  ...
  function createMicrosoftAccount(owner, scopes, clientId) {
    ...
    var acc = accountComponent.createObject(root, {
      "owner": owner,
      "clientId": clientId || "",
      "scopes": ["offline_access", "User.Read"].concat(scopes || []).join(" ")
    });
    acc.codeReceived.connect(function(code, uri) {
      editor.showNotice("Sign in to Microsoft for " + owner, "Open " + uri + " in a browser, enter this code, and sign in with your Microsoft account. This screen updates by itself once you are done.", code, [{
        "label": "Copy code",
        "icon": "󰆏",
        "action": function() {
          root.copyText(code);
  ...
  function queueFor(key, provider) {
    ...
    var q = queueComponent.createObject(root, {
      "domain": key,
      "paused": !root.opened
    });
    if (!q) {
  ...
  function applyProviderSettings(p) {
    var entry = (root.config.providers || {
    })[p.id];
    for (var k in entry) {
      if (k === "enabled" || !(k in p))
        continue;
  ...
  function addProvider(url) {
    ...
    if (p.searchChanged)
      p.searchChanged.connect(function() {
        root.invalidateContentSearch(p);
      });

    p.statusRequested.connect(function(t) {
      root.showStatus(t);
    });
    p.noticeRequested.connect(function(title, text, code, actions) {
      editor.showNotice(title, text, code, actions);
    });
    p.noticeCleared.connect(function() {
      editor.clearNotice();
    });
    ...
    p.persistRequested.connect(function() {
      root.saveState();
    });
  ...
  function orderProviderIds(ids, cfg) {
    var known = (cfg && cfg.providers) || {
    }, order = Object.keys(known), rank = {
    };
  ...
  function loadProviders(externalDirs) {
    ...
    var ids = root.orderProviderIds(entries.map(function(x) {
      return x.id;
    }), root.config);
  ...
  function reorderProviders() {
    ...
    root.providers = ids.map(function(id) {
      return byId[id];
    });
  }
  ...
  function askProvider(p, q, seq) {
    ...
    p.search(q, function(r) {
      if (seq !== root.searchSeq || root.searchRequests[p.id] !== request || !root.filterText || root.providers.indexOf(p) < 0)
        return ;
  ...
  function noteExists(path) {
    ...
    eachSection(function(prov, s, key) {
      ...
      if ((s.rows || []).some(function(r) {
        return r.kind === "note" && r.path === path;
      })) {
        found = true;
        return ;
      }
      if ((s.notes || []).some(function(n) {
        return n.path === path;
      }))
        found = true;
  ...
  function crumbOf(path) {
    ...
    return p ? p.crumb(path) : "";
  }

  // IPC helpers (omarchy-shell shell call <id> scrollList 400).
  function activateSection(key) {
    setActiveSection(String(key));
    return root.activeSection;
  }

  function tabsInfo(x) {
    return JSON.stringify(root.tabs.map(function(t) {
      return Object.assign({
        "matches": root.tabMatches[t.key] || 0
      }, t);
    }));
  }

  function scrollList(y) {
    list.setScrollOffset(Number(y));
  ...
  function debugState() {
    ...
    for (var q = 0; q < root.queueList.length; q++) {
      lanes.push({
        "key": root.queueList[q].domain,
        "depth": root.queueList[q].depth,
    ...
    return JSON.stringify({
      ...
      "rowWrites": root.rowWrites,
      "offset": Math.round(list.scrollOffset()),
      "loadFailed": root.loadFailed,
      "queues": lanes,
      "providers": root.providers.map(function(p) {
        return p.id;
      })
    });
  }
  ...
  function providerWithRow(kind, id) {
    ...
    eachSection(function(prov, s, key) {
      var matchesRow = (s.rows || []).some(function(r) {
        return r.kind === kind && r.path === id;
      });
      var matchesFooter = kind === "action" && (s.footerActions || []).some(function(action) {
  ...
  function rowsOf(providerId) {
    return JSON.stringify(root.rows.filter(function(r) {
      return r.provider === providerId;
    }).map(function(r) {
      return r.kind + ":" + r.path.substring(0, 24);
    }));
  }

  function sectionsOf(providerId) {
    var p = providerById(providerId);
    return p ? JSON.stringify(p.sections.map(function(s) {
      return s.key + "(" + s.rows.length + ")";
    })) : "no provider";
  }
  ...
  function editorCursor(pos) {
    editor.setCursorPosition(Number(pos));
    editor.updateInTable();
    return editor.cursorPosition() + (editor.inTable ? " in-table" : " outside");
  }
  ...
  function sectionKeyOf(path) {
    var found = "";
    eachSection(function(prov, s, key) {
      if (!found && (s.rows || []).some(function(r) {
        return r.kind === "note" && r.path === path;
      }))
        found = key;
  ...
  function saveInFlight(path) {
    return session.saveInFlight(path);
  }
  ...
  function writeState() {
    ...
    var st = {
      ...
      "lastNotes": root.lastNotes,
      "providers": root.providerState
    };
    if (root.listWidth > 0)
  ...
  function initialize() {
    if (!Platform.backend)
      throw new Error("Install the host platform before initializing the workspace");
  ...
  Timer {
    id: contentSearchTimer

    interval: 350
    onTriggered: root.runContentSearch()
  }

  Timer {
    id: cachedSearchTimer

    interval: 300
    onTriggered: root.refreshCachedSearch()
  }
  ...
  Item {
    ...
    Column {
      ...
      TitleBar {
        ...
        detached: root.detached
        supportsOverlay: root.supportsOverlay
      ...
      Row {
        ...
        NoteList {
          ...
          onActivated: function(path) {
            root.choosePath(path);
            editor.focusEditor();
          }
          ...
          onFooterActionRequested: function(action, value) {
            root.runFooterAction(action, value);
          }
          onActionRequested: function(id) {
            root.runAction(id);
          }
          onTreeToggled: function(id) {
            root.treeToggle(id);
          }
        ...
        Item {
          ...
          Rectangle {
            anchors.fill: parent
            color: Util.alpha(root.foreground, splitterArea.pressed ? 0.35 : (splitterArea.containsMouse ? 0.22 : 0.08))
